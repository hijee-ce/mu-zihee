...
public class MusicalController {
    ...
    public ResponseEntity<ResponseDto> selectAllMusicalList(@RequestParam(name="offset", defaultValue = "1") String offset){

        int totalCount = musicalService.selectAllMusicalTotal();
        int limit = 6;
        int buttonAmount = 5;
        ...
        responseDtoWithPaging.setData(musicalService.selectMusicalList(selectCriteria));

        return ResponseEntity.ok().body(new ResponseDto(HttpStatus.OK, "전체 뮤지컬 조회 성공", responseDtoWithPaging));
    }
...
//        musicalDto.setMusicalCode(musicalCode);
        System.out.println("PUT 메소드 진입" + musicalDto);
